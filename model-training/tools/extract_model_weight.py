...
def merge_configs(model_config_path, data_config_path):
    ...
    merged_config = {"model_cfg": model_config, "data_cfg": data_config}
...
def extract_weights(config_file=None, checkpoint_path=None, output_root="features"):
    ...
    # Create output directory under the requested feature root.
    output_dir = os.path.join(output_root, version_name)
    ...
    with open(config_yaml_path, "w") as f:
        yaml.dump(config_output, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default=None, help="Path to configuration file (auto-discovered if not specified)"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="features",
        help="Directory in which to create the extracted encoder folder",
    )
    args = parser.parse_args()

    extract_weights(args.config, args.checkpoint, args.output_root)
