...
def process_wave_features(
    ...
    for batch_idx in range(batch_size):
        patch_count = int((input_ids[batch_idx] == wave_patch_token_id).sum().item())
        feature_count = int(input_wave_embeds.shape[1])
        if patch_count != feature_count:
            raise ValueError(
                "Wave token / feature length mismatch for sample "
                f"{batch_idx}: found {patch_count} <wave_patch> tokens in input_ids "
                f"but encoder produced {feature_count} wave embeddings. "
                "Check wave_token_len and radar encoder sequence length."
            )
