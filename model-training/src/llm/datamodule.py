...
class ConversationWaveDataset(torch.utils.data.Dataset):
    """Tokenization/conversation wrapper on top of supervision-level data."""

    def __init__(
        self,
        base_dataset,
        format_conversation_fn,
        preprocess_multimodal_wave_fn,
        preprocess_with_conversation_fn,
        tokenizer,
        conversation_template,
        mm_use_wave_start_end,
        wave_token_len,
        is_inference,
        embed_wave_features,
    ):
...
class WaveLLMDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage='fit'):
        ...
        if stage == 'fit':
            ...
            self.val_dataset = self._load_dataset(self.val_split, stage='val')
    ...
    def _load_dataset(self, split, stage='train'):
        ...
        """
        if stage == 'train':
            caption_loading_mode = self.cfg.get(
                'caption_loading_mode_train',
                self.cfg.get('caption_loading_mode', None)
            )
        else:
            caption_loading_mode = self.cfg.get(
                'caption_loading_mode_val',
                self.cfg.get('caption_loading_mode', None)
            )
        ...
        original_dataset = WaveCaptionDataset(
            ...
            supervision_mode=self.cfg.get('supervision_mode', None),
            caption_loading_mode=caption_loading_mode,
            use_random_question_for_caption=self.cfg.get('use_random_question_for_caption', True),
            static_bg_npz=self.cfg.get('static_bg_npz', None),
            range_drop_prob=self.cfg.get('range_drop_prob', 0.0),
            doppler_drop_prob=self.cfg.get('doppler_drop_prob', 0.0),
            azimuth_drop_prob=self.cfg.get('azimuth_drop_prob', 0.0),
        )

        return ConversationWaveDataset(
            original_dataset,
    ...
    def _pad_and_stack_wave_views(self, wave_embeds, view_names=None):
        """
        Pad and stack wave embeddings for configured views.

        If view_names is None, infer the view set from the batch.  This allows
        uDoppler-only batches to contain only `doppler_time` while preserving
        backward compatibility with historical three-view batches.
        """
        if view_names is None:
            canonical = ['range_time', 'doppler_time', 'azimuth_time']
            present = set()
            for embed in wave_embeds:
                present.update(embed.keys())
            view_names = [name for name in canonical if name in present]
            if not view_names:
                raise ValueError("No radar views found in wave_embeds")
